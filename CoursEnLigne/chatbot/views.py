...
def chatbot_response(request): 
    if request.method == "POST": 
        try: 
            ...
            user_message = data.get("message")  # Extraire le message utilisateur depuis le JSON 
            ...
            response = requests.post( 
                "http://localhost:11434/api/chat",  # URL de l'API de chat 
                json=message_payload,  # On envoie le message formaté 
            ...
            if response.status_code == 200: 
                ...
                return JsonResponse( 
                    {"error": "Failed to fetch response from API", "status_code": response.status_code}, 
                    status=500 
                ) 
